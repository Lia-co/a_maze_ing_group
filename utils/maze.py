...
class maze:
    ...
    def __init__(self, width: int, height: int, entry: tuple[int, int], exit: tuple[int, int]) -> None:
        ...
        if width <= 0:
            raise ValueError("WIDTH is invalid, expect input: postive digits")
        if height <= 0:
            raise ValueError("HEIGHT is invalid, expect inpur: postive digits")
        ...
        # generate a grid of cells with nested list structure
        # each cell is initialized as 15
